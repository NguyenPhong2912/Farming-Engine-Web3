...
namespace FarmingEngine
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        {
            // --- Web3: Block controls if wallet not set ---
            if (!Web3WalletValidator.IsWalletValid())
            {
        ...
        private Vector3 FindMovementDirection()
        {
            ...
            // --- Web3: Block movement if wallet not set ---
            if (!Web3WalletValidator.IsWalletValid())
                return tmove;
        ...
        {
            // --- Web3: Block interactions if wallet not set ---
            if (!Web3WalletValidator.IsWalletValid())
                return;
            ...
            else if (freerotate)
            {
                ...
                && (auto_move || character_combat.HasRangedWeapon() || character_combat.IsAttackTargetInRange(target)))
            {
                selectable.Select();
                Attack(target);
            }
