...
app.post('/api/mint', async (req, res) => {
  try {
    ...
    // Sui not configured - return error instead of fake data
    console.error('[Mint] ❌ Sui not configured. Cannot mint NFT.');
    return res.status(500).json({ 
      error: 'Sui blockchain not configured',
      details: 'SUI_PACKAGE_ID and SUI_PRIVATE_KEY must be set in environment variables',
      envVars: {
...
app.get('/api/nfts', async (req, res) => {
  try {
    ...
    console.warn('[NFTs] ⚠️ Sui not configured or query failed. Returning empty list.');
    return res.json({ items: [] });
  } catch (err) {
