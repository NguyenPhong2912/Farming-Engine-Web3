...
Thêm vào Netlify Environment Variables:

- `SUI_NETWORK`: `testnet` hoặc `mainnet`
- `SUI_PACKAGE_ID`: Package ID của smart contract
- `SUI_PRIVATE_KEY`: Private key của wallet để mint (hoặc dùng service account)
...
4. Copy Package ID và thêm vào Netlify env vars
